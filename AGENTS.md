...
- Service worker is cache-first with hardcoded `CACHE_NAME = "grocery-v2"` in `sw.js`.
...
- Sync protocol: ntfy.sh topic == room name (from `#room=` URL hash, else
  ...
  `DELETE_ITEM`, `PUT_PRODUCT`, `DELETE_PRODUCT`, `REQUEST_SYNC`, `FULL_SYNC`
  (sends `{items, products}`); own messages filtered via `SENDER_ID`.
...
- Domain vocabulary lives in `CONTEXT.md`, architectural decisions in `docs/adr/`.
  ...
  (Clearing and the Purchase-history record are not implemented yet).
